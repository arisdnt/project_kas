import { useEffect, useMemo, useState } from 'react'
import { Input } from '@/core/components/ui/input'
import { Button } from '@/core/components/ui/button'
import { useKategoriStore } from '@/features/kategori/store/kategoriStore'
import { Plus, Search, X, RotateCcw } from 'lucide-react'

type Props = {
  onCreate: () => void
}

export function KategoriToolbar({ onCreate }: Props) {
  const { setSearch, loadFirst } = useKategoriStore()
  const itemsCount = useKategoriStore((state) => state.items.length)
  const totalCount = useKategoriStore((state) => state.all.length)
  const loading = useKategoriStore((state) => state.loading)
  const [query, setQuery] = useState('')

  useEffect(() => {
    const id = setTimeout(() => {
      setSearch(query)
      loadFirst()
    }, 300)
    return () => clearTimeout(id)
  }, [query, setSearch, loadFirst])

  const hasQuery = query.trim().length > 0
  const toolbarStatus = useMemo(() => {
    if (loading) return 'Memuat kategori…'
    if (totalCount === 0) return 'Tidak ada kategori yang tersimpan.'
    return `Menampilkan ${itemsCount} dari ${totalCount} kategori`
  }, [loading, itemsCount, totalCount])

  return (
    <div className="space-y-3">
      <div className="flex w-full flex-col gap-2 lg:flex-row lg:items-center lg:gap-3">
        <div className="relative w-full lg:w-[90%]">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          <Input
            placeholder="Cari nama atau deskripsi kategori"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="h-9 rounded-lg border-slate-200 pl-9 text-[13px]"
            aria-label="Cari kategori"
          />
          {hasQuery && (
            <button
              onClick={() => setQuery('')}
              className="absolute right-2 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-100"
              aria-label="Kosongkan pencarian"
            >
              <X className="h-3.5 w-3.5" />
            </button>
          )}
        </div>
        <div className="flex w-full justify-end lg:w-[10%]">
          <Button onClick={onCreate} size="sm" className="h-9 gap-1 rounded-lg px-3 text-[13px]">
            <Plus className="h-4 w-4" />
            <span className="hidden md:inline">Tambah Kategori</span>
          </Button>
        </div>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-2 text-[12px] text-slate-500">
        <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-600">
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 font-semibold text-emerald-700">
            <span className="relative flex h-2 w-2 items-center justify-center">
              <span className="absolute h-full w-full animate-ping rounded-full bg-emerald-400 opacity-60" />
              <span className="relative h-2 w-2 rounded-full bg-emerald-500" />
            </span>
            Live {new Date().toLocaleTimeString('id-ID', {
              hour: '2-digit',
              minute: '2-digit',
              second: '2-digit',
            })}
          </div>
          <span>{toolbarStatus}</span>
        </div>
        {hasQuery && (
          <button
            onClick={() => setQuery('')}
            className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 font-medium text-slate-600 transition hover:border-slate-300 hover:bg-slate-100"
          >
            <RotateCcw className="h-3.5 w-3.5" />
            Bersihkan pencarian
          </button>
        )}
      </div>
    </div>
  )
}
