...
type PelangganStatusFilterProps = {
...
export function PelangganStatusFilter({ value = 'all', onChange }: PelangganStatusFilterProps) {
  ...
  return (
    <DropdownMenu>
      ...
      >
        ...
        <DropdownMenuSeparator />
        <DropdownMenuRadioGroup value={value} onValueChange={(val) => onChange(val as PelangganStatusFilterProps['value'])}>
          {STATUS_OPTIONS.map(({ value: statusValue, label }) => (
            <DropdownMenuRadioItem key={statusValue} value={statusValue} className="cursor-pointer">
              {label}
            </DropdownMenuRadioItem>
          ))}
        </DropdownMenuRadioGroup>
