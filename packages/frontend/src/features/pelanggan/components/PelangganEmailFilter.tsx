...
type PelangganEmailFilterProps = {
...
export function PelangganEmailFilter({ value, onChange, withEmailCount, withoutEmailCount }: PelangganEmailFilterProps) {
  ...
  const displayLabel = (() => {
    ...
    if (selectedCount === 1) return value![0] === 'with-email' ? 'Dengan email' : 'Tanpa email'
    return 'Semua email'
  })()
