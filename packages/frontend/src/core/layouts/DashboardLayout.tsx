...
export function DashboardLayout() {
  ...
  // Check if current page is kasir
  const isKasirPage = location.pathname === '/dashboard/kasir';

  // Listen for global calculator open events
  useEffect(() => {
    const handleOpenCalculator = () => {
      openCalculator();
    };

    window.addEventListener('app:openCalculator', handleOpenCalculator);
    return () => window.removeEventListener('app:openCalculator', handleOpenCalculator);
  }, [openCalculator]);
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className={`flex-1 min-h-0 ${!isKasirPage ? 'py-3 sm:py-4 px-2 sm:px-4 lg:px-6 pb-32' : ''}`}>
