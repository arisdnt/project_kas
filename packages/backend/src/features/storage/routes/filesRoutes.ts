...
router.post('/upload', authorize(UserRole.ADMIN), singleFileMiddleware, FilesController.upload)

// Dapatkan pre-signed URL via redirect 302
router.get('/:key', authorize(UserRole.ADMIN, UserRole.CASHIER), FilesController.resolve)

// Dapatkan pre-signed URL sebagai JSON (untuk fetch Authorization)
router.get('/:key/url', authorize(UserRole.ADMIN, UserRole.CASHIER), FilesController.getUrl)

// Hapus file dari object storage
router.delete('/:key', authorize(UserRole.ADMIN), FilesController.delete)
