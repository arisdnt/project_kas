...
# Login untuk mendapatkan token dengan tenant_id
echo "Login sebagai admintoko dengan tenant_id..."
...
# Ekstrak accessToken dari response JSON
TOKEN=$(echo $LOGIN_RESPONSE | grep -o '"accessToken":"[^"]*"' | cut -d'"' -f4)

if [ -z "$TOKEN" ]; then
  echo "Login gagal atau token tidak ditemukan!"
...
for i in {1..10}; do
  ...
  # Buat request JSON dengan tanggal_lahir sebagai string
  JSON_DATA=$(cat <<EOF
{
  "kode": "$KODE",
  "nama": "$NAMA",
...
EOF
  ...
  if [ "$HTTP_CODE" = "201" ] || [ "$HTTP_CODE" = "200" ]; then
    ...
    # Hentikan jika ada error untuk debugging
    if [ $i -le 3 ]; then
      echo "Menghentikan untuk debugging..."
      break
    fi
  fi
